...
def fill_template_from_jsonl(jsonl_file_path, output_file_path, max_entries=None):
    ...
    for i, row in enumerate(jsonl_data):
        ...
        custom_facet['ioi-ext:channel'] = row.get('Channel', '')
        custom_facet['ioi-ext:keywords'] = row.get('Keywords', '')
        custom_facet['ioi-ext:processId']['@value'] = str(row.get('ProcessId', 0))
        custom_facet['ioi-ext:threadId']['@value'] = str(row.get('ThreadId', 0))
        custom_facet['ioi-ext:chunkNumber']['@value'] = str(row.get('ChunkNumber', 0))
        custom_facet['ioi-ext:extraDataOffset']['@value'] = str(row.get('ExtraDataOffset', 0))
        custom_facet['ioi-ext:hiddenRecord']['@value'] = str(row.get('HiddenRecord', False)).lower()
        custom_facet['ioi-ext:sourceFile'] = source_filepath
        custom_facet['ioi-ext:payloadData1'] = row.get('PayloadData1', '')
        custom_facet['ioi-ext:payloadData2'] = row.get('PayloadData2', '')
