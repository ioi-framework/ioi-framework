...
def fill_template_from_csv(csv_file_path, output_file_path=None):
    ...
    for i, row in enumerate(csv_data):
        ...
        lnk_facet['ioi-ext:targetFilePath'] = local_path
        lnk_facet['ioi-ext:targetFileName'] = target_file_name

        # Only add timestamps if they have valid data
        target_created = parse_timestamp(row.get('TargetCreated'))
        if target_created:
            lnk_facet['ioi-ext:targetCreatedTime']['@value'] = target_created
        else:
            del lnk_facet['ioi-ext:targetCreatedTime']

        target_modified = parse_timestamp(row.get('TargetModified'))
        if target_modified:
            lnk_facet['ioi-ext:targetModifiedTime']['@value'] = target_modified
        else:
            del lnk_facet['ioi-ext:targetModifiedTime']

        target_accessed = parse_timestamp(row.get('TargetAccessed'))
        if target_accessed:
            lnk_facet['ioi-ext:targetAccessedTime']['@value'] = target_accessed
        else:
            del lnk_facet['ioi-ext:targetAccessedTime']

        lnk_facet['ioi-ext:relativePath'] = row.get('RelativePath', '')
        lnk_facet['ioi-ext:workingDirectory'] = row.get('WorkingDirectory', '')
        lnk_facet['ioi-ext:arguments'] = row.get('Arguments', '')
        
        # MFT entry numbers - only include if valid
        try:
            mft_entry = row.get('TargetMFTEntryNumber', '').strip()
            if mft_entry:
                lnk_facet['ioi-ext:targetMftEntryNumber']['@value'] = int(mft_entry, 16)
            else:
                del lnk_facet['ioi-ext:targetMftEntryNumber']
        except:
            del lnk_facet['ioi-ext:targetMftEntryNumber']
        ...
        try:
            ...
            if mft_seq:
                lnk_facet['ioi-ext:targetMftSequenceNumber']['@value'] = int(mft_seq, 16)
            else:
                del lnk_facet['ioi-ext:targetMftSequenceNumber']
        except:
            del lnk_facet['ioi-ext:targetMftSequenceNumber']

        lnk_facet['ioi-ext:machineId'] = row.get('MachineID', '')
        lnk_facet['ioi-ext:machineMacAddress'] = row.get('MachineMACAddress', '')
