...
Fills MFT templates from CSV data using only custom dfc-ext properties.
...
def fill_template_from_csv(csv_file_path, output_file_path):
    ...
    for i, row in enumerate(csv_data):
        ...
        custom_facet['dfc-ext:mftFlags']['@value'] = 'false'
        ...
        for csv_field, placeholder in field_mapping.items():
            ...
            if placeholder == 'NAME_TYPE':
                ...
                custom_facet['dfc-ext:nameType'] = value or 'Unknown'
            elif placeholder == 'ENTRY_NUMBER':
                custom_facet['dfc-ext:entryNumber']['@value'] = sanitize_int(value)
            elif placeholder == 'SEQUENCE_NUMBER':
                custom_facet['dfc-ext:sequenceNumber']['@value'] = sanitize_int(value)
            elif placeholder == 'PARENT_ENTRY_NUMBER':
                custom_facet['dfc-ext:parentEntryNumber']['@value'] = sanitize_int(value)
            elif placeholder == 'PARENT_SEQUENCE_NUMBER':
                custom_facet['dfc-ext:parentSequenceNumber']['@value'] = sanitize_int(value)
            elif placeholder == 'FILE_SIZE':
                custom_facet['dfc-ext:fileSize']['@value'] = sanitize_int(value)
            elif placeholder == 'REFERENCE_COUNT':
                custom_facet['dfc-ext:referenceCount']['@value'] = sanitize_int(value)
            elif placeholder == 'SI_FLAGS':
                # Keep as string value (Archive, Hidden|System, etc.)
                custom_facet['dfc-ext:siFlags'] = value or 'None'
            elif placeholder == 'UPDATE_SEQUENCE_NUMBER':
                custom_facet['dfc-ext:updateSequenceNumber']['@value'] = sanitize_int(value)
            elif placeholder == 'LOGFILE_SEQUENCE_NUMBER':
                custom_facet['dfc-ext:logfileSequenceNumber']['@value'] = sanitize_int(value)
            elif placeholder == 'IN_USE':
                custom_facet['dfc-ext:inUse']['@value'] = value.lower() if value else 'false'
            elif placeholder == 'IS_DIRECTORY':
                custom_facet['dfc-ext:isDirectory']['@value'] = value.lower() if value else 'false'
            elif placeholder == 'HAS_ADS':
                custom_facet['dfc-ext:hasAds']['@value'] = value.lower() if value else 'false'
            elif placeholder == 'IS_ADS':
                custom_facet['dfc-ext:isAds']['@value'] = value.lower() if value else 'false'
            elif placeholder == 'SI_LESS_THAN_FN':
                custom_facet['dfc-ext:siLessThanFn']['@value'] = value.lower() if value else 'false'
            elif placeholder == 'USEC_ZEROS':
                custom_facet['dfc-ext:uSecZeros']['@value'] = sanitize_int(value)
            elif placeholder == 'COPIED':
                custom_facet['dfc-ext:copied']['@value'] = value.lower() if value else 'false'
            elif placeholder == 'CREATED_0X10':
                custom_facet['dfc-ext:created0x10']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'CREATED_0X30':
                custom_facet['dfc-ext:created0x30']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'LAST_MODIFIED_0X10':
                custom_facet['dfc-ext:lastModified0x10']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'LAST_MODIFIED_0X30':
                custom_facet['dfc-ext:lastModified0x30']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'LAST_RECORD_CHANGE_0X10':
                custom_facet['dfc-ext:lastRecordChange0x10']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'LAST_RECORD_CHANGE_0X30':
                custom_facet['dfc-ext:lastRecordChange0x30']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'LAST_ACCESS_0X10':
                custom_facet['dfc-ext:lastAccess0x10']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'LAST_ACCESS_0X30':
                custom_facet['dfc-ext:lastAccess0x30']['@value'] = sanitize_timestamp(value)
            elif placeholder == 'PARENT_PATH':
                custom_facet['dfc-ext:parentPath'] = value or ''
            elif placeholder == 'FILE_NAME':
                custom_facet['dfc-ext:fileName'] = value or ''
            elif placeholder == 'FILE_EXTENSION':
                custom_facet['dfc-ext:extension'] = value or ''
            elif placeholder == 'REPARSE_TARGET':
                custom_facet['dfc-ext:reparseTarget'] = value or ''
            elif placeholder == 'SECURITY_ID':
                custom_facet['dfc-ext:securityId'] = value or ''
            elif placeholder == 'OBJECT_ID_FILE_DROID':
                custom_facet['dfc-ext:objectIdFileDroid'] = value or ''
            elif placeholder == 'LOGGED_UTIL_STREAM':
                custom_facet['dfc-ext:loggedUtilStream'] = value or ''
            elif placeholder == 'ZONE_ID_CONTENTS':
                custom_facet['dfc-ext:zoneIdContents'] = value or ''
            elif placeholder == 'SOURCE_FILE':
                custom_facet['dfc-ext:sourceFile'] = value or ''
    ...
    print(f"   - Uses only custom dfc-ext properties (no warnings!)")
...
def fill_template_chunked(csv_file_path, output_base_path, chunk_size):
    ...
    for chunk_idx, start in enumerate(range(0, len(csv_data), chunk_size)):
        ...
        for row in chunk_data:
            ...
            field_mapping = {
                'EntryNumber': 'dfc-ext:entryNumber', 'SequenceNumber': 'dfc-ext:sequenceNumber',
                'ParentEntryNumber': 'dfc-ext:parentEntryNumber', 'ParentSequenceNumber': 'dfc-ext:parentSequenceNumber',
                'UpdateSequenceNumber': 'dfc-ext:updateSequenceNumber', 'LogfileSequenceNumber': 'dfc-ext:logfileSequenceNumber',
                'FileSize': 'dfc-ext:fileSize', 'ReferenceCount': 'dfc-ext:referenceCount',
                'uSecZeros': 'dfc-ext:uSecZeros',
            }
            bool_fields = {'InUse': 'dfc-ext:inUse', 'IsDirectory': 'dfc-ext:isDirectory',
                           'HasAds': 'dfc-ext:hasAds', 'IsAds': 'dfc-ext:isAds',
                           'SI<FN': 'dfc-ext:siLessThanFn', 'Copied': 'dfc-ext:copied'}
            ts_fields = {'Created0x10': 'dfc-ext:created0x10', 'Created0x30': 'dfc-ext:created0x30',
                         'LastModified0x10': 'dfc-ext:lastModified0x10', 'LastModified0x30': 'dfc-ext:lastModified0x30',
                         'LastRecordChange0x10': 'dfc-ext:lastRecordChange0x10', 'LastRecordChange0x30': 'dfc-ext:lastRecordChange0x30',
                         'LastAccess0x10': 'dfc-ext:lastAccess0x10', 'LastAccess0x30': 'dfc-ext:lastAccess0x30'}
            str_fields = {'ParentPath': 'dfc-ext:parentPath', 'FileName': 'dfc-ext:fileName',
                          'Extension': 'dfc-ext:extension', 'ReparseTarget': 'dfc-ext:reparseTarget',
                          'SecurityId': 'dfc-ext:securityId', 'ObjectIdFileDroid': 'dfc-ext:objectIdFileDroid',
                          'LoggedUtilStream': 'dfc-ext:loggedUtilStream', 'ZoneIdContents': 'dfc-ext:zoneIdContents',
                          'SourceFile': 'dfc-ext:sourceFile', 'NameType': 'dfc-ext:nameType',
                          'SiFlags': 'dfc-ext:siFlags'}
