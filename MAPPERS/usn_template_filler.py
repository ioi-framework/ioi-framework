...
def fill_template_from_data(input_file_path, output_file_path):
    ...
    for i, row in enumerate(json_data):
        ...
        # Fill UsnFacet properties (custom dfc-ext)
        usn_facet['dfc-ext:entryNumber']['@value'] = sanitize_int(row.get('EntryNumber', 0))
        usn_facet['dfc-ext:sequenceNumber']['@value'] = sanitize_int(row.get('SequenceNumber', 0))
        usn_facet['dfc-ext:parentEntryNumber']['@value'] = sanitize_int(row.get('ParentEntryNumber', 0))
        usn_facet['dfc-ext:parentSequenceNumber']['@value'] = sanitize_int(row.get('ParentSequenceNumber', 0))
        usn_facet['dfc-ext:parentPath'] = parent_path
        usn_facet['dfc-ext:updateSequenceNumber']['@value'] = sanitize_int(row.get('UpdateSequenceNumber', 0))
        usn_facet['dfc-ext:updateTimestamp']['@value'] = update_timestamp
        usn_facet['dfc-ext:updateReasons'] = row.get('UpdateReasons', '')
        usn_facet['dfc-ext:fileAttributes'] = row.get('FileAttributes', '')
        usn_facet['dfc-ext:offsetToData']['@value'] = sanitize_int(row.get('OffsetToData', 0))
        usn_facet['dfc-ext:sourceFile'] = source_filename
...
def fill_template_chunked(input_file_path, output_base_path, chunk_size):
    ...
    for chunk_idx in range(total_chunks):
        ...
        for row in chunk_rows:
            ...
            usn_facet['dfc-ext:fileAttributes'] = row.get('FileAttributes', '')
            usn_facet['dfc-ext:offsetToData']['@value'] = sanitize_int(row.get('OffsetToData', 0))
            usn_facet['dfc-ext:sourceFile'] = '$J'
